...
import { listingDisplayName } from "../services/domain.js";
...
export default async function statsRoutes(fastify: FastifyInstance) {
  fastify.get("/stats", async (_request, reply) => {
    ...
    const [totals, byCategory, newest, top, onlineNow, cloudflare] = await Promise.all([
      ...
      Listing.findOne({ status: "active" }).sort({ createdAt: -1 }).select("domain url createdAt").lean(),
      Listing.findOne({ status: "active" }).sort({ totalPaid: -1 }).select("domain url totalPaid").lean(),
    ...
    return reply.send({
      ...
      onlineNow,
      newestDomain: newest ? listingDisplayName(newest.url, newest.domain) : null,
