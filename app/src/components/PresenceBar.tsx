...
export default function PresenceBar() {
  const { data, isLoading } = usePresenceStats();
  ...
  return (
    <div className="inline-flex items-center gap-1.5 sm:gap-2 rounded-full border border-border bg-surface-soft px-2.5 sm:px-3.5 py-1 text-[11px] sm:text-[12px] text-ink-soft max-w-full">
      {isLoading && !data ? (
        <>
          <span className="h-3 w-16 rounded bg-black/[0.06] animate-pulse" />
          <span className="h-3 w-20 rounded bg-black/[0.06] animate-pulse" />
        </>
      ) : (
        <>
          ...
          <span className="whitespace-nowrap">
            <span className="amount font-medium text-ink">{clicks.toLocaleString()}</span> clicks
          </span>
          ...
          <Link
            to="/stats"
            className="hidden sm:inline font-medium text-ink-soft hover:text-ink transition-colors whitespace-nowrap"
          >
            see stats &rarr;
          </Link>
        </>
